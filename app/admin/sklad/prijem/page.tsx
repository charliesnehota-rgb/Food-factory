...
export default function PrijemPage() {
  ...
  const previewNet = validLines.reduce((s, l) => s + Number(l.qty) * Number(l.price), 0);
  const previewVat = validLines.reduce((s, l) => s + Number(l.qty) * Number(l.price) * Number(l.vat_rate) / 100, 0);
  ...
  function buildItemsPayload() {
    return validLines.map((l) => {
      ...
      return {
        ...
        unit_price_net_czk: toBaseUnitPrice(Number(l.price), du.factor),
  ...
  async function save(thenPost: boolean) {
    ...
    setSaving(true);
    const res = await fetch("/api/sklad/receipts", {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ ...head, supplier_id: head.supplier_id || null, items: buildItemsPayload() }),
    });
    const created = await res.json();
    if (!res.ok) { setSaving(false); alert(created.error ?? "Chyba"); return; }
    if (thenPost && created.id) {
      const pr = await fetch(`/api/sklad/receipts/${created.id}/post`, { method: "POST" });
  ...
  return (
    <div>
      <div className="mb-5 flex items-center justify-between flex-wrap gap-3">
        ...
        <button onClick={() => setOpen(!open)} className="rounded-lg bg-white px-4 py-2 text-sm font-medium text-black hover:bg-neutral-200">+ Nový příjem</button>
      </div>
        ...
        <div className="mb-6 rounded-2xl border border-[var(--border)] bg-[var(--card)] p-5">
          ...
          <div className="mt-4 space-y-2">
            <div className="hidden gap-2 text-xs text-[var(--muted)] sm:grid sm:grid-cols-[2fr_1fr_1fr_1fr_1fr_auto]">
              <span>Surovina</span><span>Množství</span><span>Jednotka</span><span>Cena/j. (bez DPH)</span><span>DPH %</span><span></span>
            </div>
            {lines.map((l, idx) => {
              const it = itemById(l.stock_item_id);
          ...
          <div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-[var(--border)] pt-4">
            <div className="text-sm text-[var(--muted)]">
              Bez DPH: <span className="text-white">{formatCzk(previewNet)}</span> · DPH: <span className="text-white">{formatCzk(previewVat)}</span> · Celkem: <span className="font-semibold text-white">{formatCzk(previewNet + previewVat)}</span>
            </div>
