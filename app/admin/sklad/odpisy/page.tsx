...
const REASONS = ["expirace", "poškození", "ztráta/krádež", "jiné"];
...
export default function OdpisyPage() {
  ...
  async function submit() {
    ...
    setItem(""); setQty(""); setNote(""); setReason(REASONS[0]); load();
  }

  return (
    <div>
      <div className="mb-5">
        <h1 className="text-xl font-semibold">Odpisy</h1>
        <p className="text-sm text-[var(--muted)]">Vyřazení surovin mimo prodej (expirace, poškození…). Oceňuje se aktuálním váženým průměrem.</p>
      </div>
      ...
      <div className="mb-6 rounded-2xl border border-[var(--border)] bg-[var(--card)] p-4">
        <div className="flex flex-wrap items-end gap-2">
          ...
          <span className="flex items-center gap-1">
            <input type="number" placeholder="množství" value={qty} onChange={(e) => setQty(e.target.value)} className={inputCls + " w-28"} />
            <span className="text-sm text-[var(--muted)]">{itemObj?.base_unit ?? ""}</span>
          </span>
          <select value={reason} onChange={(e) => setReason(e.target.value)} className={inputCls}>
            {REASONS.map((r) => <option key={r} value={r}>{r}</option>)}
          </select>
          <input placeholder="poznámka" value={note} onChange={(e) => setNote(e.target.value)} className={inputCls} />
          <button onClick={submit} disabled={saving} className="rounded-lg bg-white px-4 py-1.5 text-sm font-medium text-black hover:bg-neutral-200 disabled:opacity-50">{saving ? "…" : "Zapsat odpis"}</button>
        </div>
        {itemObj && Number(qty) > 0 && (
          <p className="mt-2 text-xs text-[var(--muted)]">Hodnota odpisu ≈ {formatCzk(Number(qty) * Number(itemObj.avg_price_czk))}</p>
        )}
      </div>
      ...
      <div className="overflow-x-auto rounded-2xl border border-[var(--border)] bg-[var(--card)]">
        <table className="w-full text-sm">
          <thead className="border-b border-[var(--border)] text-left text-[var(--muted)]">
            <tr>
              ...
              <th className="p-3 font-medium">Množství</th>
        ...
        {rows.length === 0 && !loading && <div className="p-8 text-center text-[var(--muted)]">Zatím žádné odpisy.</div>}
      </div>
    </div>
  );
}
