...
export default function KartyPage() {
  ...
  async function remove(it: StockItem) {
    if (!confirm(t("karty.deleteConfirm", { name: it.name }))) return;
  ...
  return (
    <div>
      <div className="mb-5 flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-xl font-semibold">{t("karty.title")}</h1>
      ...
      {open && (
        <div className="mb-5 rounded-2xl border border-[var(--border)] bg-[var(--card)] p-5">
          <h2 className="mb-3 font-medium">{editingId ? t("karty.editTitle") : t("karty.newTitle")}</h2>
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="sm:col-span-2">
              <label className="text-xs text-[var(--muted)]">{t("karty.labelName")}</label>
              <input value={form.name ?? ""} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="např. Hovězí mleté 20 %" className={inputCls} />
            </div>
            <div>
              <label className="text-xs text-[var(--muted)]">{t("karty.labelCategory")}</label>
            ...
            <div>
              <label className="text-xs text-[var(--muted)]">{t("karty.labelUnit")}</label>
            ...
            <div>
              <label className="text-xs text-[var(--muted)]">{t("karty.labelMinQty")} (v {form.base_unit})</label>
              <input type="number" value={form.min_qty ?? 0} onChange={(e) => setForm({ ...form, min_qty: Number(e.target.value) })} className={inputCls} />
            </div>
            <div>
              <label className="text-xs text-[var(--muted)]">{t("karty.labelTargetQty")} (v {form.base_unit})</label>
            ...
            <div>
              <label className="text-xs text-[var(--muted)]">{t("karty.labelSku")}</label>
          ...
          {!editingId && (
            <div className="mt-3 rounded-lg border border-[var(--border)] p-3">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <div>
                  <label className="text-xs text-[var(--muted)]">{t("karty.labelInitQty")} (v {form.base_unit})</label>
                  <input type="number" value={initQty} onChange={(e) => setInitQty(e.target.value)} className={inputCls} placeholder="0" />
                </div>
                <div>
                  <label className="text-xs text-[var(--muted)]">{t("karty.labelInitPrice")} ({form.base_unit})</label>
      ...
      <div className="overflow-x-auto rounded-2xl border border-[var(--border)] bg-[var(--card)]">
                ...
                <Fragment key={it.id}>
                  <tr className="border-b border-[var(--border)] last:border-0">
                    <td className="p-3">
                      ...
                      {!it.is_active && <span className="ml-2 text-xs text-[var(--muted)]">({t("karty.inactive")})</span>}
                      {it.sku && <div className="text-xs text-[var(--muted)]">{it.sku}</div>}
                      {it.category && <div className="text-xs text-[var(--muted)]">{it.category.name}</div>}
                    </td>
                    <td className="p-3">
                      <span className={low ? "text-amber-400 font-medium" : ""}>{formatQty(Number(it.current_qty), it.base_unit)}</span>
                      {low && <span className="ml-2 rounded-full bg-amber-500/15 px-2 py-0.5 text-[10px] text-amber-400">{t("karty.filterLow")}</span>}
                    </td>
                    ...
                    <td className="p-3">
                      <div className="flex gap-1">
                        <button onClick={() => startEdit(it)} className="rounded-lg border border-[var(--border)] px-2.5 py-1 text-xs text-[var(--muted)] hover:text-white hover:border-neutral-600">{t("common.edit")}</button>
                        <button
                        ...
                        >
                          {loadingUsage === it.id ? "…" : t("karty.usages")}
                        </button>
                        <button onClick={() => remove(it)} className="rounded-lg border border-[var(--border)] px-2.5 py-1 text-xs text-[var(--muted)] hover:text-red-400 hover:border-red-500/30">{t("common.delete")}</button>
                      </div>
                    </td>
                  </tr>
                  {isExpanded && (
                      ...
                      <td colSpan={5} className="px-3 pb-3 pt-1">
                        <div className="rounded-lg border border-[var(--border)] p-3">
                          {loadingUsage === it.id ? (
                            ...
                            <p className="text-xs text-[var(--muted)]">{t("karty.usages.none")}</p>
                          ) : (
                            <>
                              <div className="mb-2 text-xs font-medium uppercase tracking-wide text-[var(--muted)]">
                                {t("karty.usages.title")} ({(usages[it.id] ?? []).length})
                              </div>
                              <table className="w-full text-xs">
                                <thead className="text-left text-[var(--muted)]">
                                  <tr>
                                    <th className="p-1.5">{t("karty.usages.col.product")}</th>
                                    <th className="p-1.5">{t("karty.usages.col.concept")}</th>
                                    <th className="p-1.5">{t("karty.usages.col.qty")}</th>
                                    <th className="p-1.5">{t("karty.usages.col.price")}</th>
                                    <th className="p-1.5">{t("karty.usages.col.available")}</th>
                                  </tr>
                                </thead>
                                <tbody>
        ...
        {visible.length === 0 && !loading && <div className="p-8 text-center text-[var(--muted)]">{t("karty.empty")}</div>}
      </div>
    </div>
  );
}
