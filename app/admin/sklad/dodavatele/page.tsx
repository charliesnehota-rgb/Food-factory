...
export default function DodavatelePage() {
  ...
  async function remove(id: string) {
    ...
    await fetch(`/api/sklad/suppliers/${id}`, { method: "DELETE" });
    load();
  }
    ...
    <div>
      <div className="mb-5 flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-[var(--muted)]">{rows.length} dodavatelů{loading ? " · načítám…" : ""}</p>
        </div>
        <button onClick={startNew} className="rounded-lg bg-white px-4 py-2 text-sm font-medium text-black hover:bg-neutral-200">+ Přidat dodavatele</button>
      </div>

      {open && (
        <div className="mb-5 rounded-2xl border border-[var(--border)] bg-[var(--card)] p-5">
          <h2 className="mb-3 font-medium">{editingId ? "Upravit dodavatele" : "Nový dodavatel"}</h2>
          <div className="grid gap-3 sm:grid-cols-2">
            <Field label="Název *"><input value={form.name ?? ""} onChange={(e) => setForm({ ...form, name: e.target.value })} className={inputCls} /></Field>
            <Field label="IČO"><input value={form.ico ?? ""} onChange={(e) => setForm({ ...form, ico: e.target.value })} className={inputCls} /></Field>
            <Field label="DIČ"><input value={form.dic ?? ""} onChange={(e) => setForm({ ...form, dic: e.target.value })} className={inputCls} /></Field>
            ...
            <Field label="Adresa"><input value={form.address ?? ""} onChange={(e) => setForm({ ...form, address: e.target.value })} className={inputCls} /></Field>
            <div className="sm:col-span-2"><Field label="Poznámka"><input value={form.note ?? ""} onChange={(e) => setForm({ ...form, note: e.target.value })} className={inputCls} /></Field></div>
          </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--card)]">
        <table className="w-full text-sm">
          <thead className="border-b border-[var(--border)] text-left text-[var(--muted)]">
            <tr><th className="p-3 font-medium">Dodavatel</th><th className="p-3 font-medium">IČO / DIČ</th><th className="p-3 font-medium">Kontakt</th><th className="p-3 font-medium">Akce</th></tr>
          </thead>
          <tbody>
            {rows.map((s) => (
              <tr key={s.id} className="border-b border-[var(--border)] last:border-0">
                ...
                <td className="p-3">
                  <div className="flex gap-1">
                    <button onClick={() => startEdit(s)} className="rounded-lg border border-[var(--border)] px-2.5 py-1 text-xs text-[var(--muted)] hover:text-white hover:border-neutral-600">Upravit</button>
                    <button onClick={() => remove(s.id)} className="rounded-lg border border-[var(--border)] px-2.5 py-1 text-xs text-[var(--muted)] hover:text-red-400 hover:border-red-500/30">Smazat</button>
                  </div>
                </td>
              </tr>
            ))}
        ...
        {rows.length === 0 && !loading && <div className="p-8 text-center text-[var(--muted)]">Zatím žádní dodavatelé.</div>}
      </div>
    </div>
  );
}
