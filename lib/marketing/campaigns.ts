...
export async function resolveCampaignRecipients(
  ...
  let list = (profiles ?? []).filter(p => p.email);
  ...
  if (segment === "brand" && conceptSlug) {
    ...
    const { data: orders } = await supabaseAdmin
      ...
      .in("user_id", ids)
      .not("user_id", "is", null);
    const brandIds = new Set((orders ?? []).map(o => o.user_id));
    list = list.filter(p => brandIds.has(p.id));
  }

  if (segment === "inactive_30") {
    // Zákazníci bez zaplacené objednávky za posledních 30 dní
    ...
    const { data: recent } = await supabaseAdmin
      ...
      .in("user_id", ids)
      .not("user_id", "is", null);
    const activeIds = new Set((recent ?? []).map(o => o.user_id));
